...
**Stand: M4 · Etappe 2c (Landtagswahlkreise: 12 Länder mit Import-Vorlagen)**
...
- **Landtagswahlkreise:** Baden-Württemberg 2026, Bremen 2023 (Wahlbereiche), Hamburg 2025, Hessen 2023, Mecklenburg-Vorpommern 2026, Niedersachsen 2022, Nordrhein-Westfalen 2022, Rheinland-Pfalz 2026, Saarland 2022, Sachsen-Anhalt 2026 und Schleswig-Holstein 2022 sind eingebaut, in der Auswahl unter „Landtagswahlkreise“ und als Schnellwahl bei Fokus auf das Land; Bayern 2023 hat eine eigene Gruppe mit Stimmkreisen und den daraus gebildeten Wahlkreisen (Regierungsbezirke). Vorlagen für die Ergebnisdateien der Landeswahlleitungen (MV `l_wahlkreise.csv`, NI `wahlergebnis.csv`, NRW `LW22_WK_insgesamt.txt`, RP `Endgueltiges_Ergebnis_LW_2026_Wahlkreise.xlsx`, BW `ltw26-ergebnisse.csv`, SH `ergebnis-download.csv` aus den Wahlbezirken summiert, ST `Ergebnisse_LT_2026.xlsx`, HE `Wahlergebnisse_Landtagswahl.csv`, BY Stimmkreise und Wahlkreise `08_10_2023_Landtagswahl_2023_…_Bayern.csv` mit Erst-, Zweit- und Gesamtstimmen (gefärbt wird nach Gesamtstimmen), HH `ergebnis-download-wahlkreis.csv` und `ergebnis-download-land.csv` aus den Stimmbezirken summiert, SL `KERG_SAARLAND.csv`, HB Wahlbereiche): Erst- und Zweitstimmen je Wahlkreis (Saarland und Bremen: eine Stimmenart „Stimmen“), Prüfung gegen das Landesergebnis, Summe aufs Land. Doppelt kodierte Umlaute in CSV-Dateien („WÃ¤hler“) werden beim Einlesen repariert. Weitere Länder folgen (Katalog in `src/data/ltw.ts`). Karten, die nur einen Teil Deutschlands zeigen (Landtagswahlkreise, Berlin, importierte Geodaten), zeigen die übrigen Länder als Umfeld (Ebene „Nachbarländer“: ein- und ausblendbar, Flächen- und Grenzfarbe, Grenzstärke).
...
Als Nächstes: M4 · Etappe 2d mit Brandenburg, Sachsen und Thüringen; danach eine Vorlage für die Regionaldatenbank (GENESIS).
